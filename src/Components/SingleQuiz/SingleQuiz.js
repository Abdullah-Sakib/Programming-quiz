...
const SingleQuiz = ({ question }) => {
  ...
  return (
    <div className="border-2 rounded-lg w-2/3 mx-auto my-9 p-8 bg-blue-300">
      <div className="flex">
      <h3 className="text-xl grow">{question.question}</h3>
      <button className="hover:text-yellow-500" onClick={showCorrectAnswer}><FontAwesomeIcon icon={faEye}></FontAwesomeIcon></button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 text-start mt-4">
        {options.map((option, idx) => (
          <label
            key={idx}
            className="border rounded-lg bg-white py-2 mb-5 mt-3"
            htmlFor={option}
          >
            <input
              className="mx-2"
